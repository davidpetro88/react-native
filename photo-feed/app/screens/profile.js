...
class Profile extends React.Component {
    ...
    saveProfile = () => {
        ...
        if(name !== '') database.ref('users').child(this.state.userId).child('name').set(name);
        if(username !== '') database.ref('users').child(this.state.userId).child('username').set(username);
                        ...
                        <View style={{flex: 1}}>
                            ...
                            { this.state.editingProfile == true ? (
                                <View style={{alignItems:'center', justifyContent:'center',
                                    ...
                                    <Text> Name: </Text>
                                    <TextInput editable={true} placeholder={'Enter your name'}
                                               onChangeText={(text) => this.setState({name: text})}
                                               value={this.state.name} style={{width:250, marginVertical: 10,
                                        padding:5, borderColor: 'grey', borderWidth: 1
                                    }}/>
                                    <Text> Username: </Text>
                                    <TextInput editable={true} placeholder={'Enter your username'}
                                               onChangeText={(text) => this.setState({username: text})}
                                               value={this.state.username} style={{width:250, marginVertical: 10,
                                        padding:5, borderColor: 'grey', borderWidth: 1
                                    }}/>
